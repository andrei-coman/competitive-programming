...
int I[1 + MAXN][1 + MAXN]; //which edge connects them
int father[1 + MAXN];
...
inline void print_minimum_matching(){ //prints the maximum matching and its cost, followed by the chosen edges
    printf("%d %d\n", flux, total);
    for(int i = 1; i <= n; i++)
        for(int j = 1; j <= m; j++)
            if(!C[i][n + j] && I[i][n + j])
...
inline void minimum_matching(int start, int finish){
    S = start, D = finish;
    bellman_ford();
    while(dijkstra()){
        int flow = INF, cost = 0;
